...
var _watch = function(args){
  ...
  function watch_function(file){
    ...
    if (fs.existsSync(file.fullPath)){
      ...
      if (yaml_parser.detect(file.fullPath)) return compile_project('dynamic file changed');
...
function compile_single_file(file_path){
  ...
  roots.compile_project(file_path, function(){
    ...
    roots.print.log('finished compiling: ' + file_path + ' (' + parseInt((ended-started)/1000) + 's)');
  });
}
...
function compile_project(reason){
  ...
  return roots.compile_project(roots.project.rootDir, function(){
    var ended = (new Date).getTime();
    roots.print.log('finished compiling entire project (' + parseInt((ended-started)/1000) + 's)');
  });
}
